...
# This device has no separate normal-boot ramdisk (recovery/root is the only
# ramdisk this tree carries, and it's flashed to the AP slot as a standalone
# recovery image, not booted as normal Android from this same build). The
# placeholder boot.img rule that satisfies this dependency lives in
# bootimg.mk instead of here -- it needs to run after INSTALLED_RECOVERYIMAGE_TARGET
# is defined, and BOARD_CUSTOM_BOOTIMG_MK (bootimg.mk) is included by
# core/Makefile well after AndroidBoard.mk is parsed, so referencing that
# variable from this file evaluates to empty and silently drops the
# ordering dependency.
